...
@section('title', 'Blog Slider')

@section('pageName', 'Blog Slider')
...
@include('reseller.blog.nav')
    ...
    <div class="col-md-8">
        @if (!empty($success))
            <h1>{{$success}}</h1>
        @endif
        <a class="btn btn-default m-b add-page" href="{{ route('reseller.blog-slider.create') }}">Add Blog slider</a>            
        
        <table class="table">
            <thead>
                <tr>
                    <th width="45%" class="p-l">Title</th>
                    <th>Visibility</th>
                    <th>Created</th>
                    <th></th>
                </tr>
            ...
            <tbody>
                @if (!$cms_slider->isEmpty())
                    @foreach ($cms_slider as $key => $slider)
                        <tr>
                            <td>{{ $slider->title }}</td>
                            <td>{{ $slider->created_at }}</td>
                            <td>{{ $slider->status == '0' ? 'Inactive' : 'Active' }}</td>
                            <td class="p-r text-right">
                                <a class="btn btn-default btn-xs" href="{{ route('reseller.blog-slider.edit', $slider->id) }}">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                @else    
