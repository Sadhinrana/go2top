...
@section('pageName', 'Blog Category')
...
@include('reseller.blog.nav')
    ...
    <div class="col-md-8">
        @if (!empty($success))
            <h1>{{$success}}</h1>
        @endif
        <a class="btn btn-default m-b add-page" href="{{ route('reseller.blog-category.create') }}">Add Blog Category</a>            
        ...
        <table class="table">
            ...
            <tbody>
                @if (!$cms_blog_category->isEmpty())
                    @foreach ($cms_blog_category as $key => $blog)
                        <tr>
                            <td>{{ $blog->name }}</td>
                            <td>{{ $blog->status == '0' ? 'Inactive' : 'Active' }}</td>
                            <td>{{ $blog->created_at }}</td>
                            <td class="p-r text-right">
                                <a class="btn btn-default btn-xs" href="{{ route('reseller.blog-category.edit', $blog->id) }}">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                @else    
