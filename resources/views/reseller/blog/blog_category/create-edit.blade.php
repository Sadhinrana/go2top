...
@section('title')
    Page {{ isset($cms_blog_category) ? 'Edit' : 'Create' }}
@endsection
...
<div class="col-md-8">
    <div class="panel panel-default">
        <div class="panel-body">
            <form action="{{ isset($cms_blog_category) ? route('reseller.blog-category.update', $cms_blog_category[0]->id) : route('reseller.blog-category.store') }}" method="post" enctype="multipart/form-data">
                    @csrf

                    @if(isset($cms_blog_category))
                        @method('PUT')
                    @endif
                    ...
                    <div class="relative">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="category_name" class="form-control default-page-name page-name" name="category_name" value="{{ old('category_name', isset($cms_blog_category) ? $cms_blog_category[0]->name : '') }}">
                            @error('category_name')
                                <span role="alert">
                    ...
                    <div class="form-group">
                        <label class="control-label" for="is_status">Status</label>    
                        <select class="form-control is-public" name="is_status" id="is_status" required>
                            <option value="1" {{ old('is_status', isset($cms_blog_category) ? $cms_blog_category[0]->status : '') == '1' ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('is_status', isset($cms_blog_category) ? $cms_blog_category[0]->status : '') == '0' ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('is_status')
                            <span role="alert">
                                <strong>{{ $message }}</strong>
                ...
                <a class="btn btn-default" href="{{ route('reseller.blog-category.index') }}">Cancel</a>
            ...
            <form id="delete-form-submit" action="{{ route('reseller.blog-category.destroy', $cms_blog_category[0]->id) }}" method="POST" style="display:none">
                @csrf
                @method('DELETE')
            </form>
            @endif
