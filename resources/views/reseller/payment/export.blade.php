        ...
        <div class="col-12">
            <div class="material-card card">
                <div class="card-body">
                    ...
                    <div class="material-card card">
                        <div class="card-body">
                            <form method="post" action="{{ route('reseller.payments.export') }}" novalidate>
                                        ...
                                        <div class="form-group">
                                            <div class="controls">
                                                <select name="payment_method_ids[]" id="payment_method_ids" class="form-control select2 @error('payment_method_ids') is-invalid @enderror" required data-validation-required-message="This field is required" multiple>
                                                    <option value="all" {{ old('payment_method_ids') ? in_array('all', old('payment_method_ids')) ? 'selected' : '' : 'selected' }}>All methods</option>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="zero_config" class="table table-striped border table-hover">
                                    ...
                                    <tbody>
                                    @foreach(auth()->guard('reseller')->user()->exported_payments as $item)
                                    <tr>
                                        <td>{{ $item->from }}</td>
                                        <td>{{ $item->to }}</td>
                                        <td>{{ rtrim(implode(', ', array_map(function ($value) { return ucfirst($value); }, unserialize($item->user_ids)))) }}</td>
                                        <td>{{ rtrim(implode(', ', array_map(function ($value) { return ucfirst($value); }, unserialize($item->payment_method_ids)))) }}</td>
                                        ...
                                        <td>
                                            <form method="post" action="{{ route('reseller.payments.exported_payment.download', $item->id) }}">
                                                @csrf
